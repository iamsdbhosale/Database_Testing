...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 14, 2025 at 10:59 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCustomerShipping` (IN `pCustomerNumber` INT, OUT `pShipping` VARCHAR(50))   begin
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_order_by_cust` (IN `cust_no` INT, OUT `shipped` INT, OUT `canceled` INT, OUT `resolved` INT, OUT `disputed` INT)   begin
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SelectAllCustomersByCity` (IN `mycity` VARCHAR(50))   Begin
	select * from customers where city=mycity;
End$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SelectAllCustomersByCityandPin` (IN `mycity` VARCHAR(50), IN `pcode` VARCHAR(15))   begin
	select * from customers where city=mycity and postalCode=pcode;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `test_error` ()   BEGIN
